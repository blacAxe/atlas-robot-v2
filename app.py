...
active_transport: Literal["serial", "udp"] = "serial"
...
async def api_connect(request: ConnectRequest):
    ...
    try:
        if request.transport == "serial":
            ...
            udp_reader.disconnect()
            serial_reader.connect(request.port, request.baud)
            active_transport = "serial"
        else:
            serial_reader.disconnect()
            udp_reader.connect(request.udp_port)
...
async def api_command(request: CommandRequest):
    if active_transport != "udp":
        raise HTTPException(status_code=400, detail="Remote commands require Wi-Fi UDP mode.")
    try:
        udp_reader.send_command(request.command)
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    await websocket.send_json({"message_type": "initial_status", "data": full_status()})
    try:
        while True:
